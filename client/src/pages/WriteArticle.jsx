  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Handle successful request but logic failure (e.g. backend caught an error)
        toast.error(data.message || "Failed to generate article")
      }
    } catch (error) {
      // Check for 429 specifically (Too Many Requests)
      if (error.response?.status === 429) {
        toast.error("API Rate limit reached. Please wait 60 seconds.");
      } else {
        // Pull the specific error message from the response if available
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index) => (
            <span 
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700 border-blue-200' : 'text-gray-500 border-gray-300'}`} 
              key={index}
            >
              {item.text}
            </span>
